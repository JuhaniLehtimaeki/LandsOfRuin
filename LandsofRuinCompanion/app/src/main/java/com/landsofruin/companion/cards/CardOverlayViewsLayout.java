...
public class CardOverlayViewsLayout extends FrameLayout {
    ...
    private HashMap<CharacterCard, View> characterToView = new HashMap<>();


    private boolean isEnemyCardLayout = false;
    private View currentInFront = null;
    private boolean inScreenTakeOverMode = false;

    private boolean useElevation = false;

    public CardOverlayViewsLayout(Context context) {
        this(context, null);
        initDimensions();
    }

    public CardOverlayViewsLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        initDimensions();
    }

    public CardOverlayViewsLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void bringFront(CharacterCard card) {
        currentInFront = card.getView();
    }

    public void resetFrontCard() {
        currentInFront = null;
    }


    public void setupChildrenOrderTracking(LinkedList<CharacterCard> characterCards) {
//        if (!order.isEmpty()) {
//            return;
//        }
    ...
    public void animateToLocations() {
        ...
        float cumulativeY = (offset20 * 2) + (spreadCardMargin * (order.size() - 1));
